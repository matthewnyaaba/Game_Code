...
// Load sound effects
const winSound = new Audio('win.mp3'); // Make sure the file is in the same directory or adjust the path
...
function startTimer() {
    timer = setInterval(function() {
        if (timeLeft <= 0) {
            ...
            loseSound.play();  // Play the "lose" sound when time's up
...
function checkGuess() {
    ...
    if (userGuess < secretNumber) {
        ...
        wrongSound.play();  // Play the "wrong" sound when the guess is too low
    } else if (userGuess > secretNumber) {
        document.getElementById("message").innerText = `Too high! Try again. Attempts: ${attempts}`;
        wrongSound.play();  // Play the "wrong" sound when the guess is too high
    } else {
        ...
        winSound.play();  // Play the "win" sound when the user guesses correctly
